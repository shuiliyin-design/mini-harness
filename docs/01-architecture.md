...
`--self-check` 的实现放在根目录
[`mini_harness_self_check.py`](../mini_harness_self_check.py)，而不是 core 内。这样验证入口可以组合
...
因此 `test_complete_core_dependency_graph_is_a_dag`、layering tests 和 no-lazy-import test 都属于
