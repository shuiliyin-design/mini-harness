...
Slice A–E已实现durable multi-turn protocol、atomic ACTIVE relation + two typed promises、manual Briefing worker、
Definition structured reliability与manual Fake relation publisher，并已通过863个全仓deterministic tests。代码与
current demo DB均为schema v13且旧历史可读。daemon/scheduler、真实broker、Delivery outbox、shared Briefing
与distributed lease仍是planned。
...
| [`20-subscription-agent-harness.md`](20-subscription-agent-harness.md) | Subscription Agent Harness：Slice A–E current、typed outbox/workers、commit/crash matrix与planned边界 |
| [`21-phase35-architecture-review.md`](21-phase35-architecture-review.md) | Phase 3.5 freeze review：真实产品链、Harness ownership、state/outbox complexity与下一阶段排序 |
