...
- 完整 suite 分布在 `tests/unit/`、`tests/integration/`、`tests/e2e/`、`tests/security/` 和
  `tests/architecture/`；它们仍混合 regression 与 adversarial 场景，而不是 pure unit tests 的计数表。
...
- self-check：[`tools/self_check.py`](../../tools/self_check.py)，`run_self_check`、
  `print_self_check`。
...
- **“unittest 数量就是 pure unit test 数量。”错误。** 分类目录不改变 suite 中仍有 regression/adversarial 的事实。
